...
- Filtering strategy: generation and legendary data comes from the species endpoint; pre-fetch or cache a full index (id → generation, is_legendary, is_mythical) in a Route Handler
- `lib/` helpers: `filterByType`, `filterByGeneration`, `filterByLegendary` — pure functions
